...
public class TuyaDecoder extends ByteToMessageDecoder {
    ...
    public void decode(@NonNullByDefault({}) ChannelHandlerContext ctx, @NonNullByDefault({}) ByteBuf in,
        ...
        if ((returnCode & 0xffffff00) != 0) {
            ...
            payload = ((protocol == V3_4) || (protocol == V3_5)) ? new byte[payloadLength - 32]
                    : new byte[payloadLength - 8];
        } else {
            payload = ((protocol == V3_4) || (protocol == V3_5)) ? new byte[payloadLength - 32 - 8]
        ...
        if (commandType == UDP) {
            ...
            byte[] decodedMessage = (protocol == V3_4 || protocol == V3_5)
                    ? CryptoUtil.decryptAesEcb(payload, sessionKey, true)
